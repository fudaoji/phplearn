...
class MySendMail {
    ...
    */
    public function setMailInfo($config) {
        ...
        $this->_to = $config['to'];
        $this->_subject = "=?UTF-8?B?".base64_encode($config['subject'])."?=";  //解决中文乱码
...
$config = array(
        "from" => "461960962@qq.com",
        "to" => "3081832523@qq.com",
        "subject" => "test",
        "body" => "<b>test</b>",
        "username" => "461960962@qq.com",
        "password" => "zxbcrwqycirmbich",
        "isHTML" => true
    );
$mail = new MySendMail();
$mail->setServer("smtp.qq.com");
$mail->setMailInfo($config);
if(!$mail->sendMail()) {
    echo $mail->error();
    return 1;
